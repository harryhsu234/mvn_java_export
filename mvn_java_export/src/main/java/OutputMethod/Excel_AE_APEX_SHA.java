...
public class Excel_AE_APEX_SHA extends OutputCommon {

	
	public static String programmeTitle = "精銳科技_轉檔(上海)";
	...
	protected void doExcel() throws Exception {
		...
		for(InvItem obj : this.alInvItem) {
			...
			int col_pos = 0;
			this.setValue(row_pos, col_pos++, obj.itemSeq);
	...
	public Excel_AE_APEX_SHA() {
		// TODO Auto-generated constructor stub
		programmeTitle = "精銳科技_轉檔(上海)";
	}
	...
	protected void extractExcelSheet(String fileName, XSSFSheet xws) throws Exception {
		...
		if (xwsSheetName.startsWith("INV") && xwsSheetName.contains("報")) {
			extractINVtoArrayList(xws);
		} else if (xwsSheetName.startsWith("INV") && fileName.contains("廈門")) {
			extractINVtoArrayList(xws);
		} 
		else if (xwsSheetName.startsWith("PKG")) {
			extractPKGtoArrayList(xws, fileName);
		} else {
			System.out.println(xwsSheetName + " NO WORK!!");
			// extractDESCtoArrayList(xws);
		}
	}

	protected void extractINVtoArrayList(XSSFSheet xws) throws Exception {
		System.out.println("執行 INVOICE 頁簽2 : " + xws.getSheetName());
		...
		while (rowIterator.hasNext()) {
			...
			String prefix = "*商標:";
			if (cell.getCellTypeEnum() == CellType.STRING
					&& cell.getStringCellValue().replace(" ", "").trim().startsWith(prefix)) {
			...
			prefix = "稅則";
			if (cell.getCellTypeEnum() == CellType.STRING
					&& cell.getStringCellValue().replace(" ", "").trim().contains(prefix)) {
				String sLine = cell.getStringCellValue().replace(" ", "").trim();
				sCCCcode = sLine.substring(sLine.length() - 15, sLine.length()); // .replace(prefix, "");

				String cccCodeHead = "*項次";
				String cccCodeFoot = "稅則";
				if (sLine.contains(cccCodeHead) && sLine.contains(cccCodeFoot)) {
					// 特殊處理
					String invNos = sLine.substring(sLine.indexOf(cccCodeHead) + cccCodeHead.length(), sLine.indexOf(cccCodeFoot));
					invNos = invNos.replace("、", ",");	
					if(invNos.length() < 1) 
						htCccCode.put(0, sCCCcode);
					else {
		...
		while (rowIterator.hasNext()) {
			...
			if (startItem) {
				if (row.getCell(4).getNumericCellValue() > 0) {
					...
					String desc1 = sBigDesc.substring(0, slashPos);
					String desc2 = sBigDesc.substring(slashPos + 1, sBigDesc.length());

					String sStatistic_mode = "";
					if (sDrno_type.trim().equals(""))
						sStatistic_mode = "";
					else {
						try {
					...
					alInvItem.add(new InvItem(invItemKey, sPO_No, sBigDesc, desc1, desc2, iQty, iUnit_price, iAmt,
							sDrno_type, sStatistic_mode, sDesc, sQtyUnit, sTerm, sCurr, sTrademark, sCCCcode));
	...
	protected void extractPKGtoArrayList(XSSFSheet xws, String fileName) {
		...
		while (rowIterator.hasNext()) {
			...
			if(startItem) {
				...
				int gw_col = 5; 
				if(fileName.contains("廈門")) gw_col = 4;
					
				double gw = (double)row.getCell(gw_col).getNumericCellValue()-1;
				if(gw <= 0) gw=0.5;
				
				double GwPerItem = gw / qty;
				
				String key = itemName;
				
				htPKGItem.put(key, GwPerItem);
			}
		}
	}
