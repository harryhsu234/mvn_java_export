...
public class Excel_C3_BENQ extends OutputCommon {

	public final static String programmeTitle = "C3_BENQ_轉出程式";
	
	XSSFWorkbook wb;
	XSSFSheet ws;
	
	public Excel_C3_BENQ() {
	...
	public void getExcel(String custom_no, String booking_no) throws Exception {
		...
		// get xlsx template
		String templatePath = "/Excel_C3_BENQ.xlsx";
        ...
        outputFileName = "C3_BENQ_"+ custom_no_fileName + "_" + System.currentTimeMillis()+".xlsx";
		...
		while(rsLines.next()) {
			...
			for(String col_name : colNames_Lines) {
				...
				if(col_name.equalsIgnoreCase("SKIP_ONE")) {
					...
					if(_DESCRIPTION.toUpperCase().contains("BOM NO.")) {
						_DESCRIPTION = _DESCRIPTION.toUpperCase().split("BOM NO.")[0].trim(); // 不顯示BOM NO. 相關資訊
					}
				...
				else if(col_name.equalsIgnoreCase("INV_UM")) {
					String _INV_UM = rsLines.getString("INV_UM");
					if(_INV_UM.trim().equalsIgnoreCase("PCS")) {
						_INV_UM = "PCE"; // 不顯示BOM NO. 相關資訊
					}
					
					this.setValue(_line_row, col_pos++, _INV_UM);
					...
					String _DESCRIPTION = rsLines.getString("DESCRIPTION");
					if(_DESCRIPTION.toUpperCase().contains("BOM NO.")) {
						String _BOM = _DESCRIPTION.toUpperCase().split("BOM NO.")[1].trim();
